      ...
      with engine.connect() as conn:
        conn.execute(text("""
          CREATE TABLE IF NOT EXISTS event_costs (
            ...
            restroom_paidout_total NUMERIC DEFAULT 0,
            coat_check_paidout_total NUMERIC DEFAULT 0,
            bartender_wages_total NUMERIC DEFAULT 0,
            barback_wages_total NUMERIC DEFAULT 0,
            server_wages_total NUMERIC DEFAULT 0,
            created_at TIMESTAMP DEFAULT NOW(),
            updated_at TIMESTAMP DEFAULT NOW()
          )
        """))
        ...
        with engine.connect() as conn:
            ...
            event = conn.execute(text("""
                SELECT e.*,
                  ...
                  n.door_presold_revenue, n.door_walkup_revenue,
                  n.table_presold_revenue, n.table_walkup_revenue
                FROM events e
                LEFT JOIN post_event_reviews r ON r.event_id = e.id
                LEFT JOIN night_of_actuals n ON n.event_id = e.id AND n.time_of_entry ILIKE 'close%'
                WHERE e.id = :eid
