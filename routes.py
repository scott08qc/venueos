    ...
    def get_promoter_intelligence(promoter: str, event_type: str = None):
      ...
      with engine.connect() as conn:
        ...
        return {
          ...
          "score_components": {
            "draw_accuracy": {"score": draw_score, "avg_draw_ratio": avg_draw_ratio, "label": f"Averages {round((avg_draw_ratio or 0) * 100)}% of promised attendance" if avg_draw_ratio else "Insufficient data", "weight": "35%"},
            "bar_yield": {"score": bar_score, "avg_spend_per_head": avg_sph, "label": f"${avg_sph}/head avg bar spend" if avg_sph else "Insufficient data", "weight": "30%"},
            "deal_profitability": {"score": profitability_score, "avg_net_revenue": avg_net, "label": f"${round(avg_net):,} avg net to venue" if avg_net else "Insufficient data", "weight": "25%"},
            "consistency": {"score": consistency_score, "label": f"Based on {len(draw_ratios)} shows" if draw_ratios else "Insufficient data", "weight": "10%"}
          },
          "recommendations": {
